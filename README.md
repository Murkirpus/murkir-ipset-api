...
Добавил nftables.php - работает с nftables - есть блокировка целых списков IP и CIDR, как по API так и в веб интерфейсе!
...
Для безопасности нужно запускать iptables.php на отдельном субдомене и на отдельной версии PHP с включенным exec.
На пример все сайты работают на PHP-7.4, а этот скрипт запускаешь на PHP-8.3 c разрешенным exec.
Добавлена возможность добавления IP в белый список - их заблокировать нельзя.
